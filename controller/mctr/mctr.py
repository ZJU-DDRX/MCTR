...
def get_boxed_indices(start_theta: float,
    ...
    # Structures
    a = aspect_ratio
    beam_proj = 2 * a * np.array([np.sin(beam_angles), np.cos(beam_angles)])
...
class DelaunayFTG:
    def __init__(
        ...
        self.scan: LaserScan = rospy.wait_for_message('/scan', LaserScan)
        while True:
            self.LIDAR_SAMPLE_IDXS, self.LIDAR_THETA_LUT = get_boxed_indices(
                self.scan.angle_min, self.scan.angle_max, len(self.scan.ranges), 41, 2.0)

            if self.LIDAR_SAMPLE_IDXS.shape[0] > 0:
                break

            rospy.logwarn("Trying again to get the lidar indices")
        ...
        rospy.Subscriber('/car_state/odom', Odometry, self._odom_callback)
    ...
    def _preprocess_lidar(self, ranges) -> np.ndarray:
        ...
        1. Setting each value to the mean over a specified window.s
    ...
    def process_lidar(self, ranges) -> tuple:
        ...
        if centerline is not None and len(centerline) > 0:
            ...
            # Search the closest lookahead_dist point
            ...
            if not self.mapping:
                speed = np.minimum(np.sqrt(self.friction_coeff*self.friction_scale* 9.81 / np.abs(kappa_lookahead)),2.5)
                ...
                rospy.loginfo("Calculated speed: %.3f", speed)
            else:
                speed = 1.25
        else:
            speed = 0
    ...
    def _extract_centerline(self, ranges, segment_idxs) -> np.ndarray:
        ...
        while not np.all(visited) and i < 1000:
            ...
            raw_points.append(points[next_idx])
        ...
        if np.linalg.norm(raw_points[-1] - raw_points[-2]) > 1 and np.sum(positive_points > 0) >= 15:
            sorted_points = sorted_points[:-6]
        elif np.sum(positive_points > 0) >= 8:
            sorted_points = sorted_points[:-2]

        ccma = CCMA(w_ma=5, w_cc=5)
        ...
        if len(sorted_x) < 3:
            rospy.logwarn("[FTG] Not enough points for Savitzky-Golay filter, skipping smoothing.")
            centerline_x_old = sorted_x
            centerline_y_old = sorted_y
        else:
            window = min(5, len(sorted_x))
            if window % 2 == 0:
                window -= 1
            poly = min(2, window - 1)
            try:
                centerline_x_old = savgol_filter(sorted_x, window_length=window, polyorder=poly)
                centerline_y_old = savgol_filter(sorted_y, window_length=window, polyorder=poly)
            except Exception as e:
                rospy.logwarn(f"[FTG] Savitzky-Golay failed: {e}")
                centerline_x_old = sorted_x
                centerline_y_old = sorted_y
        try:
            ...
            if self.DEBUG:
                self.ax.cla()
                self.ax.set_title('Centerline Extraction with CCMA')
                self.ax.set_xlabel('x [m]')
                self.ax.set_ylabel('y [m]')
                plt.triplot(og_points[:, 0], og_points[:, 1], delaunay.simplices.copy())
                plt.scatter(delaunay_midpoints[:, 0], delaunay_midpoints[:, 1], c='r', label='Midpoints')
                plt.scatter(og_points[:, 0], og_points[:, 1])
                # acatter ransacced points as squares markers
                plt.scatter(centerline_x_postrs, centerline_y_postrs, c='purple',marker="d", label='MCTR Centerline', alpha=0.5)
            ...
            rospy.logwarn(f"Error in centerline extraction: {e}")
            return None, None
