...
from purepursuit.src.PP_Controller import PP_Controller
from map.src.MAP_Controller import MAP_Controller
from ftg.ftg import FTG

from mctr.mctr import DelaunayFTG
from dtr.dtr import DelaunayFTG as Delaunay

from centerline_ftg_pp.centerline_ftg_pp import CenterlineFTG
...
class Controller_manager:
    ...
    def init_controller(self):
        ...
        #init for centerlin_ftg_controller
        self.centerline_ftg_controller = CenterlineFTG()
        rospy.loginfo(f"[{self.name}] Initialized CenterlineFTG controller")
        ...
        # Delaunay FTG
        self.mctr = DelaunayFTG(mapping=False, debug=True)
        # Delaunay
        self.dtr = Delaunay(mapping=False, debug=False)
    ...
    def scan_cb(self, data: LaserScan):
        # 检查data.ranges是否为None或空
        if data is None or data.ranges is None or len(data.ranges) == 0:
            rospy.logerr("[controller_manager] Received invalid scan data!")
            self.scan = None
        else:
            self.scan = data
    ...
    def controller_loop(self, rate: rospy.Rate):
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                elif self.state != "FTGONLY" and self.ctrl_algo == "FTG":
                    speed, steering_angle = self.ftg_cycle()
                elif self.state != "FTGONLY" and self.ctrl_algo == "MCTR":
                    speed, acceleration, jerk, steering_angle = self.mctr_cycle()
                elif self.state != "FTGONLY" and self.ctrl_algo == "DTR":
                    speed, acceleration, jerk, steering_angle = self.dtr_cycle()
                else:
                    rospy.logwarn_throttle(1.0, f"[{self.name}] No valid controller selected")
                    
            except Exception as e:
                rospy.logerr(f"[{self.name}] Error in controller loop: {e}")
...
# visualization utilities
    ...
    def mctr_cycle(self):
        ...
        This function executes one cycle of the MCTR controller.
        It uses LiDAR scan data to compute speed and steering commands,
        while setting acceleration and jerk to zero.
        
        Returns:
            tuple: (speed, acceleration, jerk, steering_angle)
        """
        if self.scan is None:
            rospy.logwarn_throttle(1.0, "[controller_manager] No scan data yet for DelaunayFTG!")
            return 0.0, 0.0, 0.0, 0.0
        
        self.mctr.set_vel(self.speed_now)
        speed, steering_angle = self.mctr.process_lidar(self.scan.ranges)
        acceleration = 0.0
        jerk = 0.0
    ...
    def dtr_cycle(self):
        ...
        This function executes one cycle of the standard DTR controller.
        It processes LiDAR scan data to compute speed and steering commands,
        updates velocity, and visualizes the steering and lookahead point if available.
        ...
        """
        if self.scan is None:
            rospy.logwarn_throttle(1.0, "[controller_manager] No scan data yet for Delaunay!")
            return 0.0, 0.0, 0.0, 0.0
        self.dtr.set_vel(self.speed_now)
        speed, steering_angle = self.dtr.process_lidar(self.scan.ranges)

        if hasattr(self.dtr, 'centerline') and self.dtr.centerline is not None and len(self.dtr.centerline) > 0:
            lookahead_dist = self.dtr.lookahead_qv * self.speed_now + self.dtr.lookahead_qm
            centerline = self.dtr.centerline
            if centerline is not None and len(centerline) > 0:
                try:
                    idx = np.argmin(np.abs(np.linalg.norm(centerline - np.array([0, 0]), axis=1) - lookahead_dist))
                    lookahead_point = centerline[idx]
                    self.set_lookahead_marker(lookahead_point, 300) 
                except Exception as e:
                    rospy.logwarn(f"[controller_manager] Error visualizing Delaunay lookahead: {e}")
